...
const formatDuration = useCallback(function(time:number,isCalledToUpdateCurrentDuration:boolean = false){
    ...
    if(videoRef.current){
       ...
       if(isCalledToUpdateCurrentDuration){
        if( hours === 0){
          setCurrentDuration(`${minutes}:${leadingZeroFormatter.format(seconds)}`);
        }
        else{
           setCurrentDuration(`${hours}:${leadingZeroFormatter.format(minutes)}:${leadingZeroFormatter.format(seconds)}`);
        }
       }
       else{
        if( hours === 0){
          setTotalDuration(`${minutes}:${leadingZeroFormatter.format(seconds)}`);
        }
        else{
           setTotalDuration(`${hours}:${leadingZeroFormatter.format(minutes)}:${leadingZeroFormatter.format(seconds)}`);
        }
       }
    }
},[]);
  ...
  const handleKeyDown = useCallback(function(e: React.KeyboardEvent<HTMLSelectElement>){
